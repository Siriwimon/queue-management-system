...
var db = mongojs('queuelog',['queuelog']);
...
var timer = schedule.scheduleJob(rule,function(){
    initProcess(function(res){
		...
		checkWaittingList(function(response){			
			waittingList = response;
			pendingList.splice(0,pendingList.length); 
			console.log("waittingList: ", waittingList);
...
function checkWaittingList(callback){
	var listBuffer = []
	var list = waittingQ.slice(0,5);
	// console.log(list);
...
app.get('/service/refresh', function(req,res){
	...
	if (pendingList == "" || pendingList == null){
		pendingQ = "";
	}else{
	...
	db.customerlog.find({}).sort({_id:-1}).limit(1).toArray(function(err,docs){
		lastRequest = [state,docs,user,queueBuffer,pendingQ,waittingList];
		res.json(lastRequest);
	});
});
...
io.sockets.on('connection', function(socket) {
	socket.on('clickService', function(message){
		...
		setTimeout(function() {
			serviceStatus = [state,queueInfo,waittingList]
	...
	socket.on('callnext', function(message){
		...
		setTimeout(function() {
			...
			// }
			serviceStatus = [queueBuffer, state, waittingList]
		...
		setTimeout(function() {
			...
			io.sockets.emit('updatewaittable', waittingList);
			// socket.broadcast.emit('oncallnext', serviceStatus);	
